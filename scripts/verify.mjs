...
//       dir-sync/           (← a symlink to this repo = the <id> dir; the
//                              extension keeps its on-disk id `dir-sync` even
//                              though the repo/listing are "directory-sync")
//
// `../../types` (the HOST contract) resolves up to .verify/types.ts — the
// production position. The server/worker halves reach the extension's own
// `messages.ts` via `../messages` (inside the repo); the ui imports
...
// ── Run the checks ─────────────────────────────────────────────────────────
console.log('Verifying directory-sync (production-nested mirror in .verify/):\n');
